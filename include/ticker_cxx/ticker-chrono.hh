...
#ifndef TICKER_CXX_TICKER_CHRONO_HH
#define TICKER_CXX_TICKER_CHRONO_HH
...
namespace ticker::chrono::detail {
...
inline void unix_time(struct timespec *spec) {
  ...
  wintime -= ticker::chrono::detail::w2ux;
  spec->tv_sec = wintime / ticker::chrono::detail::exp7;
  spec->tv_nsec = wintime % ticker::chrono::detail::exp7 * 100;
}
inline int clock_gettime(int, timespec *spec) {
  ...
  if (tps != tmp) {
    ...
    unix_time(&startspec);
    ticks2nano = (double) ticker::chrono::detail::exp9 / tps;
  }
  ::QueryPerformanceCounter((LARGE_INTEGER *) &curticks);
  curticks -= startticks;
  spec->tv_sec = startspec.tv_sec + (curticks / tps);
  spec->tv_nsec = (long) (startspec.tv_nsec + (double) (curticks % tps) * ticks2nano);
  if (!(spec->tv_nsec < ticker::chrono::detail::exp9)) {
    spec->tv_sec++;
    spec->tv_nsec -= ticker::chrono::detail::exp9;
  }
  return 0;
}
...
namespace ticker::chrono {
  ...
  /**
     ...
     * @code{c++}
     *    void yours(){
     *          ticker::chrono::high_res_duration hrd;
     *          
     *          //...
     *          
     ...
     * sample:
     * @code{c++}
     *     ticker::chrono::high_res_duration hrd([](auto duration) -> bool {
  ...
  public:
    high_res_duration(std::function<bool(std::chrono::high_resolution_clock::duration duration)> const &fn = nullptr)
        : _then(std::chrono::high_resolution_clock::now()), _cb(fn) {}
    ~high_res_duration() {
    ...
    template<typename T,
             std::enable_if_t<is_duration<T>::value, bool> = true>
...
inline std::ostream &operator<<(std::ostream &os, const ticker::chrono::iom::fmtflags v) {
  using iom = ticker::chrono::iom;
  iom::set_flags(v);
  return os;
}
...
namespace ticker::chrono {
  ...
  /**
     ...
     * if (ticker::chrono::try_parse_by(tm, time_str, "%H:%M:%S", "%Y-%m-%d %H:%M:%S", "%Y/%m/%d %H:%M:%S")) {
     *     auto tp = ticker::chrono::tm_2_time_point(&tm);
     *     // ...
     * }
     * @endcode
     */
...
template<typename T,
         std::enable_if_t<ticker::chrono::is_duration<T>::value, bool> = true>
inline std::ostream &operator<<(std::ostream &os, T const &v) {
  return ticker::chrono::format_duration(os, v);
}

template<typename T,
         std::enable_if_t<ticker::chrono::is_duration<T>::value, bool>>
inline void ticker::chrono::high_res_duration::print_duration(std::ostream &os, T v) {
  os << "It took ";
  ticker::chrono::format_duration(os, v);
  os << '\n';
}

template<class _Clock, class _Duration = typename _Clock::duration>
inline std::ostream &operator<<(std::ostream &os, std::chrono::time_point<_Clock, _Duration> const &time) {
  // std::size_t ns = ticker::chrono::time_point_get_ns(time);
  return ticker::chrono::serialize_time_point(os, time, "%F %T");
}

inline std::ostream &operator<<(std::ostream &os, std::tm const *tm) {
  return ticker::chrono::serialize_tm(os, tm, "%F %T");
}
inline std::ostream &operator<<(std::ostream &os, std::tm const &tm) {
  return ticker::chrono::serialize_tm(os, &tm, "%F %T");
}

inline std::ostream &operator<<(std::ostream &os, ticker::chrono::clock const &v) {
  return v.serialize(os, "%F %T");
}

#endif //TICKER_CXX_TICKER_CHRONO_HH
